...
import { Bell, Briefcase, CalendarDays, ChevronDown, Loader2, LogOut, ShieldCheck, Sparkles, UserRound } from "lucide-react";
...
export function Topbar({ user, onLogout, notifications, notificationError }: TopbarProps) {
  ...
  const [readNotificationIds, setReadNotificationIds] = useState<string[]>(() => loadReadNotificationIds(storageKey));
              ...
              {showProfile ? (
                  <div className="absolute right-0 mt-2 w-88 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-2xl border border-zinc-200 bg-white p-4 shadow-2xl shadow-zinc-200/60">
                    ...
                    <div className="mt-3 space-y-3 text-sm">
                      ...
                      />
                      <div>
                        <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-zinc-500">Assigned projects</p>
                        {user.assignedProjects.length > 0 ? (
                            <div className="space-y-2">
                              {user.assignedProjects.map((project) => (
                                  <div
                                      className="rounded-2xl border border-sky-100 bg-gradient-to-r from-sky-50 via-white to-indigo-50 p-3 shadow-sm"
                                      key={project.allocationId}
                                  >
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-xs font-medium text-zinc-500">{project.projectCode}</p>
                                        <div className="mt-3 grid gap-2 sm:grid-cols-2">
                                          <ProjectMeta
                                              icon={<Sparkles className="h-3.5 w-3.5" />}
                                              label="Allocation"
                                              value={formatAllocationPercent(project.allocationPercent)}
                                          />
                                          <ProjectMeta
                                              icon={<CalendarDays className="h-3.5 w-3.5" />}
                                              label="End date"
                                              value={formatDateOnly(project.allocationEndDate)}
                                          />
                                        </div>
                                      </div>
                                    </div>
                            ...
                            <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 px-3 py-3 text-sm font-medium text-zinc-600">
                              No Project Assigned (Un-allocated)
                            </div>
                        )}
                      </div>
                      <div>
