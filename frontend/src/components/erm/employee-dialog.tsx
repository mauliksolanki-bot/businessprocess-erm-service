...
import { CalendarDays, Mail, Save, ShieldCheck, UserRound, X } from "lucide-react";
...
export function EmployeeDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/45 p-3 backdrop-blur-sm sm:p-6">
      <div className="my-auto flex max-h-[calc(100vh-1.5rem)] w-full max-w-4xl flex-col overflow-hidden rounded-[28px] border border-white/40 bg-white shadow-2xl shadow-slate-900/20 sm:max-h-[calc(100vh-3rem)]">
        <div className="flex items-start justify-between border-b border-zinc-200 bg-gradient-to-r from-blue-600 via-indigo-600 to-sky-500 px-5 py-4 text-white sm:px-6 sm:py-5">
          ...
          <button aria-label="Close employee dialog" className="rounded-full bg-white/10 p-2 transition hover:bg-white/20" onClick={onClose} type="button">
            <X className="h-5 w-5" />
          </button>
        </div>
              ...
              <Card className="border-0 bg-gradient-to-br from-blue-50 via-white to-indigo-50 shadow-none">
                ...
                <CardContent className="grid gap-4">
                  ...
                  <div className="rounded-2xl bg-white/80 p-4">
                    <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Last updated</p>
                    <p className="mt-1 text-sm text-zinc-700">{formatDate(currentEmployee.updatedAt)}</p>
                  </div>
                </CardContent>
              </Card>
            ) : hasPendingRequest ? (
